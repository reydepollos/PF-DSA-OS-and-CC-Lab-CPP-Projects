...
int main() {
    ...
    cout << "Enter grammar rules (enter 'ε' for null productions):\n";
    do {
        cout << "\nEnter rule (e.g., A): ";
        cin >> rule;
        cout << "Enter productions for " << rule << " (separated by '|', e.g., ab|bc|ε): ";
        string input;
        cin >> input;
        ...
        while ((pos = input.find('|')) != string::npos) {
            ...
            if (production != "ε")  // Exclude null production
                grammar[rule].push_back(production);
            input.erase(0, pos + 1);
        }
        if (input != "ε")  // Last production, exclude null if present
